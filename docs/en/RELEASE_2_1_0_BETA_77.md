# Release `v2.1.0-beta.77` — Falcon admin chrome + daily apps
...
The admin kitchen gets Falcon-inspired chrome (tokens, top vs side nav, Apply/Save dock) plus daily apps on **our** flat-file data: **teams**, **account**, **events**, **time tracker**, **widgets**, and a **catalog side menu** that never collides with the header.
...
| **Teams** | `data/teams/{id}.json` (`team@1`) — editorial / support / ops / custom; ADMIN+ at `/platform/teams` |
| **Account** | `/account` tabs; `PUT /api/auth/me`; public opt-in staff cards `GET /api/public/staff`; content share bar |
| **Events** | `data/events/{id}.json` (`site-event@1`) — list + month calendar at `/platform/events` |
...
- [ ] `/platform/teams` create a Support team; `/account` edit public card + timezone
- [ ] `/platform/events` create a published event; `/platform/time-tracker` start a timer on a page
