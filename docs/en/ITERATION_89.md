...
> **Status:** ✅ shipped — **89a–e** complete · **`v2.1.0-beta.82`** (89b–e; 89a in **`beta.80`**)  
...
| Plugins | PHP in `Http/Extensions/{id}/` | server-side hooks/routes | ✅ scan + capabilities + broker + SafeHookRunner (It.89) |
...
| Check | Why |
|-------|-----|
| Indirect call patterns | ✅ `$fn()`, `call_user_func('exec', …)`, `array_map('system', …)` (89d) |
| `$$`, `extract()` | ✅ dynamic variable / symbol injection (89d) |
| Manifest consistency (heuristic) | ✅ declared capabilities vs `content()` / `media()` / outbound HTTP (89d) |
...
|---------|----------------|
| Error boundary | ✅ `SafeHookRunner` wraps `HookManager::run()` — `catch (Throwable)`, isolate failure (89c) |
| Per-hook quota | ✅ time + memory budget per plugin invocation (89c) |
| Auto-disable | ✅ fatal `Error` or 3 consecutive failures → `enabled=false` + audit + admin notice (89c) |
| Capability audit | ✅ `plugin {id} used {capability}` via `LogSanitizer` (89c) |
...
| **89b** | `PluginCapabilityBroker` + `PluginRuntimeContext` (content:read/write, media:read) | ✅ Hook invoke injects context; undeclared API throws `PluginCapabilityDeniedException` |
...
- [x] `plugin.json` requires validated `capabilities[]` for new imports
- [x] `PluginCapabilityBroker` is the only runtime API surface for plugins
- [x] All hook invocations pass through `SafeHookRunner`
- [x] Layer 1 scanner covers indirection patterns used in regression tests
- [x] `hello-widget` migrated as reference capability-compliant plugin
- [x] CLI scan/create documented in `docs/developer/EXTENSION_CODE_POLICY.md`
- [x] SK/EN i18n for new admin errors (import/enable blockers)
