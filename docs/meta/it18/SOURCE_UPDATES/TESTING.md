...
> Posledná aktualizácia: **2026-08-02** · ISS-120 CI log redaction · PHPUnit / PHPStan / Vitest gate
...
| **CI mirror (GitHub Actions)**                        | `.github/scripts/run-backend-tests-ci.sh` (sanitized PHPUnit log) | —                    |
...
| PHPUnit detail v logu | plný výstup (vrátane 2FA debug)                   | `--no-output` (bez echo)   |


### Test output & secret redaction

PHPUnit tests (e.g. `TwoFactorSimpleTest`) can **echo TOTP seeds, QR payloads, or tokens** into verbose output. That is acceptable on a **private local machine** but must **never** reach public GitHub job logs, issues, or git.

| Environment | Script | Published output |
|-------------|--------|------------------|
| **GitHub Actions** | `.github/scripts/run-backend-tests-ci.sh` | Sanitized only (`sanitize-ci-log.py` + `verify-ci-log-redaction.sh`) |
| **Local pre-commit** | `./scripts/iteration-gate.sh` | Terminal summary only — PHPUnit uses `--no-output` |
| **Local full regression** | `./scripts/run-all-tests.zsh` | Full log under `PAGINIUMCMS_TEST_LOG_DIR` (default `~/projects/paginiumcms_tests/`) — **outside repo** |

**Maintainer checklist (local, gitignored):** copy [`LOCAL_TEST_LOGS.md.example`](../../LOCAL_TEST_LOGS.md.example) → `LOCAL_TEST_LOGS.md` in repo root.

**Before sharing a log** (issue, PR comment, chat):

```bash
python3 .github/scripts/sanitize-ci-log.py ~/projects/paginiumcms_tests/alltests_*.log > /tmp/alltests.sanitized.log
.github/scripts/verify-ci-log-redaction.sh /tmp/alltests.sanitized.log
```

Incident and fix: [ISSUES.md](../ISSUES.md) **ISS-120**.
