...
### Documentation — Hybrid Engine Phase 0 (2026-08-02)

- **Architecture pivot:** PaginiumCMS documented as **Hybrid Headless Content Engine** with mandatory **No-SQL** SSOT.
- New (EN): [docs/architecture/NOSQL_MANDATE.md](docs/architecture/NOSQL_MANDATE.md), [HYBRID_ENGINE.md](docs/architecture/HYBRID_ENGINE.md), [DEPLOYMENT_MODES.md](docs/architecture/DEPLOYMENT_MODES.md).
- New wave (EN): [ITERATION_WAVE_HYBRID_ENGINE.md](docs/ITERATION_WAVE_HYBRID_ENGINE.md), [ITERATION_68.md](docs/ITERATION_68.md) … [ITERATION_77.md](docs/ITERATION_77.md), [ITERATION_75.md](docs/ITERATION_75.md).
- **It.74:** additive auth confirmed — admin session + CSRF unchanged.
- **It.76 + It.77:** both translation paths (LibreTranslate self-hosted + cloud APIs).
- Updated (SK): [PHILOSOPHY.md](docs/PHILOSOPHY.md), [ROADMAP.md](docs/ROADMAP.md), [CONTINUATION.md](docs/CONTINUATION.md), [STORAGE.md](docs/architecture/STORAGE.md).
- Updated (EN): [docs/README.md](docs/README.md), [FEATURE_OVERVIEW.md](docs/FEATURE_OVERVIEW.md), [ARCHITECTURE.md](docs/architecture/ARCHITECTURE.md) intro.
- **Implementation paused** until bilingual documentation pass is complete (maintainer).

### Security

- **ISS-120** — GitHub Actions PHPUnit log no longer publishes recoverable 2FA/TOTP test output; CI uses `run-backend-tests-ci.sh` + `sanitize-ci-log.py` + `verify-ci-log-redaction.sh`. Local policy: [`LOCAL_TEST_LOGS.md.example`](LOCAL_TEST_LOGS.md.example) · [TESTING.md](docs/developer/TESTING.md#test-output--secret-redaction).

### Security (audit 2026-07-31 — ISS-113–118)

- nginx security headers on static SPA (`docs/deploy/nginx-security-headers-*.conf`, `nginx-paginiumcms.com.conf`).
- CSRF exempt prefix word-boundary fix (`CsrfMiddleware`).
- `expose_php = Off` in Docker PHP image.
- `TRUSTED_PROXIES` default loopback only; explicit `.env` for LAN/prod proxy.
- `/.well-known/security.txt` (RFC 9116) in `frontend/public` + nginx location.
- **Prod ops (2026-07-31):** homelab vhost `/etc/nginx/sites-enabled/paginiumcms` — snippet alone was insufficient; required `include` in each static `location`. Documented in [NGINX_API.md](docs/deploy/NGINX_API.md#production-host-paginiumcmscom--ops-pitfalls-2026-07-31) · ISS-113.
- **Docker boot autostart (ISS-119):** `restart: unless-stopped` on prod/demo `php` + `nginx` in `docs/deploy/docker-compose.prod.yml`.

See [ISSUES.md](docs/ISSUES.md) ISS-113–118 · [SECURITY_REVIEW.md](docs/SECURITY_REVIEW.md).

### Fixed

- **ISS-112** — Lock indicator „aktívne pred 56 rokmi“: `parseContentDate` wrongly treated Unix **seconds** (post-2001) as milliseconds → ~1970. Fix + Vitest regression. Ship with next release after `v2.1.0-beta.23`.
