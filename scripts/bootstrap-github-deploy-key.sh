...
echo ""
echo "=== Docker Compose ==="
echo "  stack.sh auto-mounts this key via docs/deploy/docker-compose.deploy-key.yml"
echo "  when the file exists (PHP sees it at /run/secrets/github_deploy_key)."
echo "  Or run: STACK_DIR=/var/lib/docker/compose/paginiumcms ./scripts/ensure-php-deploy-key-mount.sh"
echo ""
