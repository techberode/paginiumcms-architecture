      ...
      <div className="card-body">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
            {t('dashboard.panels.conflicts.title')}
          </h2>
          <span className="text-sm text-gray-500">
            {t('dashboard.panels.conflicts.loggedCount', { count: String(totalCount) })}
          </span>
        </div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{t('dashboard.panels.conflicts.empty')}</p>
        ) : (
            ...
            {conflicts.map((conflict, index) => (
              ...
              >
                ...
                <p className="text-xs text-gray-500">
                  {conflict.userName} · {new Date(conflict.occurredAt * 1000).toLocaleString(dateLocale)}
                </p>
              </li>
            ))}
          </ul>
        ...
        <Link to="/audit" className="inline-block mt-3 text-sm text-indigo-600 hover:underline">
          {t('dashboard.panels.conflicts.openAudit')}
        </Link>
      </div>
    </div>
  );
