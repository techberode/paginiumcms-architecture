import type { MessageTree } from '../../types';

export const editorSk: MessageTree = {
  shell: {
    createArticle: 'Vytvoriť článok',
    createPage: 'Vytvoriť stránku',
    editArticle: 'Upraviť článok',
    editPage: 'Upraviť stránku',
    newItem: 'Nová položka',
    contentLocale: 'Jazyk obsahu',
    translateMissing: 'Preložiť chýbajúce',
    wysiwyg: 'WYSIWYG',
    preview: 'Náhľad',
    previewTitle: 'Náhľad celej stránky (header + obsah + footer)',
    previewWebTitle: 'Náhľad na webe',
    close: 'Zavrieť',
    workspace: 'Workspace',
    workspaceToggleHint: 'Celoobrazovkový editor bez admin lišty — vhodné na menší monitor',
    workspaceExit: 'Opustiť workspace',
    workspaceDetails: 'Meta',
    workspaceDetailsHint: 'SEO, menu a šablóna ostávajú v bežnom editore. Opustite workspace, ak ich potrebujete.',
    workspaceHint: 'Escape alebo Opustiť workspace vráti admin lištu.',
    title: 'Názov',
    titlePlaceholder: 'Titulok stránky alebo článku',
    slug: 'Slug',
    slugHint: 'Pri novej stránke sa slug vygeneruje z názvu. Neskôr ho môžete zmeniť — stará verejná URL prestane platiť.',
    slugChangeWarning: 'Po uložení sa zmení verejná URL. Položky v navigácii ostávajú na starej ceste, kým ich neupravíte.',
    status: 'Stav',
    template: 'Šablóna',
    layoutTemplate: 'Layout šablóna stránky',
    developerLockedHint:
      'Site layout builder je Developer, ale vaša rola nemôže použiť Monaco režim, kým je zapnuté obmedzenie ADMIN.',
    builderHelp: {
      templates:
        'Režim šablón: vyber pomenovanú štruktúru stránky. Telo ostane Markdown. Zmena: Nastavenia → Layout.',
      shortcodes:
        'Režim shortcodes: do Markdownu vkladáš tagy ako [landing-hero]. Uložené telo je rovnaké ako v Outline.',
      outline:
        'Outline: bloky z palety a polia vo formulári. Stránka sa ukladá ako Markdown. Fotky galérie berie z Galérie funkcií, nie z tohto formulára.',
      developer:
        'Developer: Markdown zdroj plus živý náhľad. Ten istý dokument ako Outline. Nové typy blokov sú v Shortcodes admin.',
    },
    description: 'Popis',
    descriptionPlaceholder: 'Krátky popis pre vyhľadávače a sociálne siete',
    routingTitle: 'Kam smeruje táto položka',
    publicUrl: 'Verejná URL:',
    menuItem: 'Položka menu:',
    notInMenu: 'Zatiaľ nie je v menu. Použite cestu :path.',
    editMenu: 'Upraviť menu',
    articleAuthor: 'Autor článku',
    articleAuthorPlaceholder: 'Prázdne = predvolený autor z Nastavenia → Obsah',
    articleAuthorHint: 'Meno autora pod titulkom. Bio a avatar nastavte v Nastaveniach → Obsah (blogAuthorName / bio), nie cez používateľský účet.',
    seoSettings: 'SEO nastavenia',
    show: 'Zobraziť',
    hide: 'Skryť',
    stats: ':characters znakov • :lines riadkov',
    cancel: 'Zrušiť',
    saving: 'Ukladám…',
    save: 'Uložiť',
    statusLabels: {
      draft: 'Koncept',
      published: 'Publikované',
      archived: 'Archivované',
      scheduled: 'Naplánované',
    },
    scheduledAt: 'Publikovať o',
    scheduledAtHint: 'Po uložení sa stav prepne na Naplánované. Cron publikuje obsah v zadanom čase.',
    templates: {
      default: 'Predvolená',
      home: 'Domov (hero)',
      about: 'O nás',
      contact: 'Kontakt',
      landing: 'Landing',
      services: 'Služby',
      blog: 'Blog',
    },
  },
  comments: {
    title: 'Diskusia k článku',
    enable: 'Povoliť diskusiu pri tomto článku',
    approval: 'Schvaľovanie príspevkov',
    guests: 'Diskusia od hostí',
    rating: 'Hodnotenie článku s komentárom',
    inheritRatingOn: 'zapnuté',
    inheritRatingOff: 'vypnuté',
    triState: {
      inherit: 'Globálne nastavenie',
      yes: 'Áno',
      no: 'Nie',
    },
    inheritApprovalOn: 'schvaľovať',
    inheritApprovalOff: 'bez schvaľovania',
    inheritGuestsOn: 'povolené',
    inheritGuestsOff: 'zakázané',
  },
  seo: {
    intro:
      'Tieto polia napájame na verejné SEO meta tagy. Ak SEO titulok necháte prázdny, použije sa názov stránky.',
    seoTitle: 'SEO titulok',
    seoTitlePlaceholder: 'Alternatívny titulok vo vyhľadávaní',
    metaDescription: 'Meta popis',
    metaDescriptionPlaceholder: 'Krátky súhrn pre vyhľadávače a sociálne siete',
    ogImage: 'OG / náhľadový obrázok',
    ogImagePlaceholder: '/storage/app/content/media/… alebo https://…',
    pickFromMedia: 'Vybrať z médií',
    removePreview: 'Odstrániť náhľad',
    ogPreviewAlt: 'Náhľad OG obrázka',
    ogHint: 'Použije sa pre sociálne siete',
    ogHintArticle: 'Použije sa pre sociálne siete a ako náhľad článku v zozname',
    canonical: 'Kanonická URL (voliteľné)',
    canonicalPlaceholder: 'Prázdne = automatická kanonická URL',
    tags: 'Tagy (oddelené čiarkou)',
    tagsPlaceholder: 'tech, cms, flat-file',
    noIndex: 'Skryť pred vyhľadávačmi (noindex)',
    pickImageTitle: 'Vybrať náhľadový obrázok',
    health: {
      okTitle: 'SEO metadáta vyzerajú v poriadku',
      okHint: 'Pred publikáciou skontrolujte titulok, popis a náhľadový obrázok.',
      draftHint: 'Tieto body budú povinné alebo odporúčané po publikovaní.',
      title: {
        warning: 'SEO odporúčania',
        critical: 'SEO chyby pred publikáciou',
      },
    },
    issues: {
      missing_description: {
        title: 'Chýba meta popis',
        hint: 'Doplňte krátky popis (120–160 znakov) pre vyhľadávače a sociálne siete.',
      },
      missing_seo_title: {
        title: 'Chýba SEO titulok',
        hint: 'Voliteľný alternatívny titulok; ak chýba, použije sa názov stránky.',
      },
      missing_og_image: {
        title: 'Chýba náhľadový / OG obrázok',
        hint: 'Pridajte obrázok pre zdieľanie na sociálnych sieťach a rich preview.',
      },
      missing_tags: {
        title: 'Chýbajú tagy článku',
        hint: 'Pridajte aspoň jeden tag pre lepšiu kategorizáciu a interné vyhľadávanie.',
      },
      title_too_long: {
        title: 'SEO titulok je príliš dlhý',
        hint: 'Odporúčame max. 60 znakov, aby sa titulok neorezal vo výsledkoch vyhľadávania.',
      },
      description_too_long: {
        title: 'Meta popis je príliš dlhý',
        hint: 'Odporúčame max. 160 znakov, aby sa popis zobrazil celý v SERP.',
      },
    },
  },
  metaSuggest: {
    title: 'Návrh meta údajov',
    hint: 'Pravidlový generátor z titulku a tela — nič sa neuloží bez tlačidla Použiť.',
    suggestTags: 'Navrhnúť tagy',
    suggestDescription: 'Generovať popis',
    loading: 'Generujem…',
    previewTags: 'Navrhované tagy',
    previewDescription: 'Navrhovaný meta popis',
    applyTags: 'Použiť tagy',
    applyDescription: 'Použiť popis',
    appliedTags: 'Tagy boli doplnené',
    appliedDescription: 'Meta popis bol nastavený',
    dismissPreview: 'Zavrieť náhľad',
    replaceWarning: 'Existujúci popis bude nahradený.',
    emptyBody: 'Zadaj aspoň titulok alebo obsah.',
    error: 'Generovanie meta údajov zlyhalo.',
  },
  markdown: {
    toast: {
      loadFailed: 'Nepodarilo sa načítať obsah',
      draftRestored: 'Koncept bol obnovený',
      titleRequired: 'Zadajte prosím názov',
      slugRequired: 'Slug nemôže byť prázdny',
      slugExists: 'Slug „:slug“ už existuje. Zvoľte iný.',
      slugChanged: 'Slug sa zmenil na „:slug“. Skontrolujte navigáciu — stará URL už neplatí.',
      otpSent: 'Overovací kód pre publikáciu bol odoslaný na email',
      devOtp: 'Dev OTP: :code',
      saved: 'Obsah bol uložený',
      draftRestoredFull: 'Koncept bol obnovený (vrátane jazykov, SEO a ďalších polí)',
      autoMerged: 'Zmeny boli automaticky zlúčené so serverovou verziou.',
      conflict: 'Konflikt (:count) – vyriešte ho prosím manuálne.',
      conflictWholeResource:
        'Konflikt (:count) – na serveri sa zmenil celý dokument (všetky locale). Vyriešte ho manuálne.',
      saveFailed: 'Uloženie zlyhalo',
      conflictCancelled: 'Riešenie konfliktu zrušené. Vaše zmeny ostali neuložené.',
      published: 'Obsah bol publikovaný',
    },
    autoSave: {
      saving: 'Ukladám koncept…',
      saved: 'Koncept uložený',
      error: 'Koncept sa nepodarilo uložiť',
      unsaved: 'Neuložené zmeny',
      leaveSaved: 'Neuložené zmeny boli uložené ako koncept',
    },
    draftBanner: {
      message: 'Našiel sa neuložený koncept. Chcete ho obnoviť?',
      restore: 'Obnoviť',
      dismiss: 'Zahodiť',
    },
    versionHistory: 'História verzií',
    commitMessage: {
      label: 'Popis zmeny (voliteľné)',
      placeholder: 'Napr. Aktualizácia úvodného odseku…',
    },
    otpPublish: {
      title: 'Potvrdiť publikáciu',
      description: 'Zadajte overovací kód z emailu pre publikovanie tohto obsahu.',
    },
    defaultAuthor: 'Redakcia',
  },
  markdownContent: {
    toolbar: {
      bold: 'Tučné',
      italic: 'Kurzíva',
      heading: 'Nadpis H2',
      link: 'Odkaz',
      image: 'Obrázok',
      video: 'Video z knižnice',
      document: 'Odkaz na dokument',
      bulletList: 'Zoznam',
      orderedList: 'Číslovaný zoznam',
      blockquote: 'Citácia',
      code: 'Kód',
      codeBlock: 'Blok kódu',
      widget: 'Vložiť widget',
    },
    prompts: {
      linkUrl: 'URL odkazu',
      imageUrl: 'URL obrázka',
    },
    insert: {
      imageAlt: 'obrázok',
      listItem: 'položka',
      quote: 'citát',
    },
    modes: {
      edit: 'Úprava',
      split: 'Rozdelený',
      preview: 'Náhľad',
    },
    placeholder: 'Píšte v Markdown… (# nadpis, **tučné**, [odkaz](url))',
    previewLabel: 'Náhľad',
    previewEmpty: 'Náhľad sa zobrazí po napísaní obsahu.',
    blockedHtmlPaste: 'Profil editora nepovoľuje vloženie raw HTML.',
    videoPickerRequired: 'Vyberte video z Media Library (zapnite Video v Nastavenia → Editor → voliteľné rozšírenia).',
    documentPickerRequired: 'Vyberte dokument z Media Library (povolte dokumenty v Nastavenia → Médiá).',
  },
  wysiwyg: {
    loading: 'Načítavanie editora…',
    prompts: {
      linkUrl: 'URL odkazu',
    },
    toolbar: {
      image: 'Vložiť obrázok z knižnice',
      video: 'Vložiť video z knižnice',
      htmlBlock: 'Vložiť trusted HTML blok',
      embed: 'Vložiť externý embed (YouTube/Vimeo)',
    },
    blocked: {
      images: 'Profil editora nepovoľuje obrázky.',
      videos: 'Profil editora nepovoľuje vloženie videa.',
      tables: 'Profil editora nepovoľuje tabuľky.',
      codeBlock: 'Profil editora nepovoľuje bloky kódu.',
      scripts: 'Profil editora nepovoľuje vložené skripty alebo iframe.',
      links: 'Profil editora nepovoľuje odkazy.',
      uploadUnavailable: 'Upload obrázkov nie je dostupný.',
    },
    defaultImageAlt: 'obrázok',
  },
  sitePreview: {
    title: 'Náhľad stránky',
    untitled: 'Bez názvu',
    typeArticle: 'článok',
    typePage: 'stránka',
    scale: {
      fullscreen: 'Celá obrazovka',
      '100': '100 %',
      '75': '75 %',
      '50': '50 %',
    },
    constrainWidth: 'Obmedziť šírku',
    fullscreen: 'Celá obrazovka',
    close: 'Zavrieť náhľad',
    defaultAuthor: 'Redakcia',
    localeHint: 'Náhľad locale :locale · stav :status (len náhľad, nie live publish)',
  },
  mediaMeta: {
    title: 'Upraviť metadáta',
    close: 'Zavrieť',
    titleLabel: 'Titulok',
    titlePlaceholder: 'Zobrazovaný titulok',
    altLabel: 'Alt text / popis',
    altPlaceholder: 'Popíšte obrázok pre prístupnosť a SEO',
    cancel: 'Zrušiť',
    saving: 'Ukladám…',
    save: 'Uložiť zmeny',
  },
  mediaPicker: {
    defaultTitle: 'Vložiť z knižnice médií',
    empty: 'V knižnici médií nie sú žiadne obrázky.',
    emptyVideo: 'V knižnici médií nie sú žiadne videá.',
    emptyDocument: 'V knižnici médií nie sú žiadne dokumenty.',
    documentTitle: 'Vložiť odkaz na dokument',
  },
  tags: {
    title: 'Tagy článku',
    hint: 'Filtrujú verejný blog a zobrazia sa na karte článku.',
    empty: 'Zatiaľ bez tagov — pridajte napr. FlatFile, Výkon, SEO.',
    removeAria: 'Odstrániť tag :tag',
    placeholder: 'Nový tag (Enter)',
    ariaLabel: 'Nový tag',
    add: 'Pridať tag',
  },
  author: {
    title: 'Autor',
    defaultLabel: 'Redakcia',
    useDefault: 'Predvolený web (:name)',
    customOption: 'Vlastný autor (ručné meno / foto)',
    previewHint: 'Zobrazí sa pri článku na webe',
    displayName: 'Zobrazované meno',
    bio: 'Bio autora (voliteľné)',
    bioPlaceholder: 'Krátke bio do boxu „O autorovi“',
    avatar: 'Fotografia autora',
    avatarPlaceholder: '/storage/… alebo https://…',
    pickAvatar: 'Vybrať z médií',
    hint: 'Vyberte používateľa CMS alebo prepíšte meno, bio a foto pre tento článok. Predvolené hodnoty sú v',
    settingsLink: 'Nastavenia → Obsah',
    usersLink: 'Používatelia',
  },
  category: {
    title: 'Kategória',
    none: 'Bez kategórie',
    hint: 'Voliteľná taxonomická skupina pre filtre blogu a sidebar widgety.',
    manageLink: 'Spravovať kategórie',
  },
  profiles: {
    label: 'Profil editora',
    company: {
      label: 'Firemná stránka',
      description: 'Základné formátovanie pre firemné texty — bez médií a pokročilých blokov.',
    },
    blog: {
      label: 'Blog',
      description: 'Články s nadpismi, obrázkami a citáciami — bez raw HTML a tabuliek.',
    },
    minimal: {
      label: 'Minimálny',
      description: 'Len základné formátovanie — vhodné pre právne texty.',
    },
  },
  previewPage: {
    loadError: 'Stránku sa nepodarilo načítať (skontrolujte prihlásenie a slug).',
    notFound: 'Stránka nenájdená',
    backToList: '← Späť na zoznam',
    bannerLabel: 'Náhľad · stav:',
    edit: 'Upraviť',
  },
  outline: {
    title: 'Bloky stránky',
    description: 'Pridajte sekcie z palety a vyplňte polia. Stránka sa stále ukladá ako Markdown.',
    livePreviewHint: 'Vpravo je živý náhľad (skripty vypnuté). Verejná stránka používa ten istý expander.',
    canvasHint: 'Ľavý stĺpec je plátno blokov. Kliknite na kartu a polia upravíte vpravo. Živý náhľad ostáva nezmenený.',
    selectBlock: 'Kliknite na blok na plátne, aby ste upravili jeho polia.',
    empty: 'Zatiaľ žiadne bloky. Pridajte hero, text alebo karty z palety.',
    add: 'Pridať',
    remove: 'Odstrániť',
    fieldsTitle: 'Polia bloku',
    customShortcode: 'Iný shortcode',
    loadFailed: 'Schémy polí shortcodov sa nepodarilo načítať. Známé hodnoty môžete upraviť aj tak.',
    proseLabel: 'Text',
    prosePlaceholder: 'Napíšte text…',
    innerLabel: 'Vnútorný obsah',
    noAttrs: 'Tento blok nemá ďalšie polia.',
    videoSrc: 'Súbor videa',
    videoPoster: 'Náhľadový obrázok',
    pickMedia: 'Vybrať z knižnice',
    clearMedia: 'Odstrániť',
    fields: {
      image: 'Hero obrázok',
      poster: 'Poster videa',
      src: 'Video na pozadí',
      srcmobile: 'Video pre mobil',
      title: 'Titulok',
      tag: 'Značka',
      subtitle: 'Podtitulok',
      cta: 'Text tlačidla',
      href: 'Odkaz tlačidla',
    },
    fieldHelp: {
      tag: 'Nálepka z Galérie funkcií (web, mail). Prázdne = všetky publikované fotky. Nie je to slug stránky.',
      title: 'Nadpis nad mriežkou pri bloku Galéria.',
    },
    raw: 'Nerozpoznaný blok',
    reorderHint: 'Presuňte karty ťahaním za úchyt alebo šípkami. Poradie sa zapíše do Markdownu.',
    dragHandle: 'Presunúť blok',
    moveUp: 'Posunúť nahor',
    moveDown: 'Posunúť nadol',
    starter: {
      portfolio: 'Portfolio starter',
      portfolioHint: 'Vloží hero, karty a výzvu k akcii. Stále ide o Markdown shortcody.',
      landing: 'Landing starter',
      landingHint: 'Vloží showcase hero, karty a výzvu k akcii.',
    },
    palette: {
      prose: 'Text',
      video: 'Video',
    },
    blocks: {
      'landing-hero': 'Hero',
      'showcase-hero': 'Showcase hero',
      'section-head': 'Nadpis sekcie',
      'feature-gallery': 'Galéria',
      'feature-grid': 'Karty',
      'cta-banner': 'Výzva k akcii',
      testimonial: 'Citát',
      'stats-row': 'Štatistiky',
      'pricing-table': 'Cenník',
      'alert-box': 'Upozornenie',
      'coming-soon': 'Čoskoro',
      'stack-grid': 'Stack značky',
      'staff-card': 'Karta používateľa',
      'staff-team': 'Karty tímu',
    },
  },
  shortcodes: {
    title: 'Vložiť shortcode',
    description: 'Vloží registrovaný shortcode tag do tela stránky. Expanzia prebehne pri renderi na serveri.',
    pick: 'Shortcode',
    insert: 'Vložiť',
    empty: 'Zatiaľ nie sú registrované shortcodes.',
    toast: {
      loadFailed: 'Nepodarilo sa načítať shortcodes',
    },
  },
  widgets: {
    title: 'Vložiť widget',
    hint: 'Vyberte kartu na verejný web, vyplňte polia a vložte markdown. Server expanduje [widget] až pri renderi.',
    insert: 'Vložiť widget',
    open: 'Otvoriť katalóg',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť widget',
  },
  snippets: {
    title: 'Vložiť snippet',
    description: 'Vloží referenciu na znovupoužiteľný snippet. Telo sa načíta z knižnice pri renderi.',
    pick: 'Snippet',
    insert: 'Vložiť',
    empty: 'Knižnica snippetov je prázdna.',
    toast: {
      loadFailed: 'Nepodarilo sa načítať snippety',
    },
  },
  htmlBlock: {
    title: 'Vložiť HTML blok',
    hint: 'HTML sa uloží do chráneného bloku :::html-safe a pri uložení sa sanitizuje. Skripty a iframe nie sú povolené.',
    placeholder: '<div class="my-block">\n  …\n</div>',
    insert: 'Vložiť blok',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť HTML blok',
  },
  embed: {
    title: 'Vložiť externý embed',
    hint: 'Prehrávač YouTube alebo Vimeo sa uloží do chráneného bloku :::embed. Ukladá sa len ID videa — nie raw iframe HTML.',
    providerLabel: 'Poskytovateľ',
    idLabel: 'ID videa',
    idPlaceholder: {
      youtube: 'dQw4w9WgXcQ',
      vimeo: '123456789',
    },
    providers: {
      youtube: 'YouTube',
      vimeo: 'Vimeo',
    },
    insert: 'Vložiť embed',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť YouTube/Vimeo',
  },
  tableInsert: {
    title: 'Vložiť tabuľku',
    hint: 'Vytvorí GFM tabuľku s placeholder bunkami na úpravu.',
    columns: 'Stĺpce',
    rows: 'Riadky dát',
    includeHeader: 'Riadok hlavičky',
    insert: 'Vložiť tabuľku',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť tabuľku',
  },
  callout: {
    title: 'Vložiť callout',
    hint: 'Poznámka, tip alebo varovanie uložené ako chránený shortcode blok.',
    typeLabel: 'Typ',
    bodyLabel: 'Text',
    bodyPlaceholder: 'Text calloutu…',
    types: {
      note: 'Poznámka',
      tip: 'Tip',
      warning: 'Varovanie',
    },
    insert: 'Vložiť callout',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť callout',
  },
  mermaid: {
    title: 'Vložiť diagram',
    hint: 'Mermaid syntax sa pri publikovaní vykreslí na serveri do SVG — bez client-side JavaScriptu.',
    sourceLabel: 'Zdroj diagramu',
    sourcePlaceholder: 'flowchart TD\n    A --> B',
    note: 'Náhľad v editore zobrazí len zdroj. Podporované typy zodpovedajú serverovému rendereru (flowchart, sequenceDiagram, …).',
    insert: 'Vložiť diagram',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť diagram',
  },
  chart: {
    title: 'Vložiť graf',
    hint: 'Zadajte popisky a hodnoty — server pri publikovaní vygeneruje statické SVG.',
    typeLabel: 'Typ grafu',
    typeBar: 'Stĺpcový graf',
    typeLine: 'Čiarový graf',
    titleLabel: 'Nadpis (voliteľný)',
    titlePlaceholder: 'Štvrtročný predaj',
    labelsLabel: 'Popisky (oddelené čiarkou)',
    labelsPlaceholder: 'Jan,Feb,Mar',
    valuesLabel: 'Hodnoty (čísla oddelené čiarkou)',
    valuesPlaceholder: '12,19,8',
    note: 'Náhľad v editore zobrazí len JSON. Max. 12 bodov; hodnoty musia byť nezáporné čísla.',
    insert: 'Vložiť graf',
    cancel: 'Zrušiť',
    toolbar: 'Vložiť graf',
  },
  versions: {
    confirmRestore: 'Obnoviť verziu {version}? Aktuálny koncept bude prepísaný.',
    confirmPrune: 'Zmazať všetky verzie okrem posledných 10 pre tento súbor?',
  },
  translation: {
    translateMissing: 'Preložiť chýbajúce',
    source: 'Zdroj',
    targets: 'Chýbajúce jazyky',
    proposing: 'Žiadam preklad…',
    applyDraft: 'Použiť ako koncept',
    discard: 'Zahodiť',
    applied: 'Preklad je uložený ako koncept. Publikácia ostáva samostatný krok.',
    disabled: 'Asistovaný preklad je vypnutý v Nastaveniach.',
    conflict: 'Dokument sa na serveri zmenil. Obnovte ho a skúste znova.',
    quota: 'Denná kvóta prekladu je vyčerpaná.',
    failed: 'Preklad zlyhal',
    empty: 'Najprv dokument uložte, až potom preložte chýbajúce jazyky.',
    reviewTitle: 'Skontrolovať návrh prekladu',
    skipped: 'Preskočené (už vyplnené)',
    failedLocale: 'Zlyhalo',
    close: 'Zavrieť',
  },
  agent: {
    title: 'AI asistent',
    hint: 'Len návrhy. Použiť zapíše vybrané polia. Agent nikdy nepublikuje.',
    suggestSeo: 'Navrhnúť SEO pre tento článok',
    running: 'Beží…',
    runStatus: '{status} · {provider}',
    proposal: 'Návrh',
    seoTitle: 'SEO titulok',
    seoDescription: 'SEO popis',
    keywords: 'Kľúčové slová',
    apply: 'Použiť vybrané',
    discard: 'Zahodiť',
    applied: 'Návrh je použitý',
    disabled: 'AI asistent je vypnutý alebo nemá žiadne tooly.',
    conflict: 'Článok sa zmenil — obnovte ho a skúste znova.',
    budget: 'Denný limit asistenta je vyčerpaný.',
    failed: 'Beh asistenta zlyhal.',
    noProposal: 'Beh skončil bez použiteľného návrhu.',
  },
};
