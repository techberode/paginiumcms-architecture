...
final class TeamChatRateLimitMiddleware extends RateLimitMiddleware
{
    /**
    ...
    public function __construct(CacheManager $cache, array $trustedProxies = [])
    {
        ...
        parent::__construct(
            $cache,
            maxRequests: $isTesting ? 100000 : ($isDevelopment ? 80 : 40),
            window: 60,
            excludedPaths: [],
    ...
    {
        return sprintf('rate_limit_team_chat:%s', md5($this->getClientIp($request)));
    }
}
